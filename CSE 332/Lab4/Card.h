...
struct card {
	...
	static string pokerString(vector<card> & cards);

	static int pokerRank( vector<card> & cards);

	static int optimalRank(const vector<card> & cards);

	static string optimalString(const vector<card> & cards);

	static int parseCards(vector<card> & cards, char * name);

	static int printCards(const vector<card> & cards);

	static void printStars(const vector<card> & cards);
};