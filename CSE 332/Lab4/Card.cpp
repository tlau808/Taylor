...
string card::pokerString(vector<card> & cards) {
...
int card::pokerRank( vector<card> & cards) {
...
int card::optimalRank(const vector<card> & cards) {
	int high = 0;
	for (int i = 1; i <= 6; ++i) {
		for (int j = 0; j < i; ++j) {
			...
			if (pokerRank(temp) > high) { high = pokerRank(temp); }
		}
	}
	return high;
}

string card::optimalString(const vector<card> & cards) {
	...
	for (int i = 1; i <= 6; ++i) {
		for (int j = 0; j < i; ++j) {
			...
			temp.erase(temp.begin() + i);
			temp.erase(temp.begin() + j);
			if (pokerRank(temp) > high) { 
				high = pokerRank(temp);
				rank = pokerString(temp);
			}
		}
	}
	return rank;
...
void card::printStars(const vector<card> & cards) {
	if (cards.size() == 0) {
		cout << "There are no valid cards to print";
	}
	cout << "* * ";
	if (cards.size() < 7) {
		for (unsigned int i = 2; i < cards.size(); ++i) {
			string suit;
			if (cards[i].suit == 1) { suit = 'c'; }
			if (cards[i].suit == 2) { suit = 'D'; }
			if (cards[i].suit == 3) { suit = 'H'; }
